...
def image_process():
    ...
    for i in range(num_photos):
        full_path = os.path.join(image_path, f"image_{i+1}.jpg")
        curr_result = image_analysis(full_path)
        if curr_result:
...
def recommend_muscles(forward_head, rounded_shoulders, pelvic_tilt, knee_hyperextension, hip_right, knee_right):
...
def postural_analysis(forward_head, rounded_shoulders, pelvic_tilt, knee_hyperextension, hip_right, knee_right):
    ...
    muscles_to_strengthen = recommend_muscles(forward_head, rounded_shoulders, pelvic_tilt, knee_hyperextension, hip_right, knee_right)
    ...
    if final_issues:
        ...
        print("No significant postural deviations detected.")
    ...
    return string_joint_changes, string_muscle_deficit

# Example usage:
# capture.capture_images()

image_data_list = image_process()
print(f"image data {image_data_list}")
average_posture, hip_right, knee_right = average_image_data(image_data_list)
...
if average_posture:
    ...
    postural_analysis(forward_head, rounded_shoulders, pelvic_tilt, knee_hyperextension, hip_right, knee_right)
